...
def test_get_metrics():
    ...
    assert performance["overall"]["precision"] == (1 + 2 / 5) / 2
    assert performance["overall"]["recall"] == (1 / 2 + 1) / 2
    assert performance["class"]["a"]["precision"] == 1
    assert performance["class"]["a"]["recall"] == 1 / 2
    assert performance["class"]["b"]["precision"] == 2 / 5
    assert performance["class"]["b"]["recall"] == 1
